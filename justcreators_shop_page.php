...
function jc_shop_save_claim( $user, $application, $member ) {
    ...
    $data = array(
        ...
        'creator_name' => $application ? $application->applicant_name : $user['username'],
        'minecraft_name' => $member ? $member->minecraft_name : '',
        'social_channels' => $application ? $application->social_channels : null,
...
function jc_shop_update_item( $shop_id, $item_key ) {
    ...
    if ( ! $item ) {
           return new WP_Error( 'invalid_item', 'Ungültiges Item.' );
    }

    if ( jc_shop_item_is_taken( $item_key, $shop_id ) ) {
        return new WP_Error( 'item_taken', 'Dieses Item ist bereits in einem anderen Shop vergeben.' );
    }
...
function jc_shop_get_social_names( $social_channels_json ) {
    $names = array();
    $channels = json_decode( $social_channels_json, true );
